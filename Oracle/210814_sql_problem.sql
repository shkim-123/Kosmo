...
select emp_no as "사원번호", emp_name as "직원명", jikup as "직급", salary||'만원' as "연봉", hire_date as "입사일"
from employee
--18. employee 테이블에서 직원명, 직급, 연봉, 세금, 실수령액을 검색하면? (세금은 연봉의 12%)
select emp_name, jikup, salary, salary*0.12, salary*0.88
from employee
...
select min(salary), max(salary), avg(salary), sum(salary), count(*)
from employee
...
select emp_no, emp_name, substr(jumin_num, 3, 2)||'-'||substr(jumin_num, 5, 2)
from employee
...
select emp_no, emp_name, jikup, decode(substr(jumin_num, 7, 1), '1', '남', '3', '남', '여') from employee
...
select emp_no, emp_name
    , floor((extract(year from sysdate)
        - to_number(decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(jumin_num, 1, 2)) + 1)*0.1)
from employee
...
select emp_no, emp_name
    ...
    , extract(year from hire_date)
        - to_number(decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(jumin_num, 1, 2)) + 1   "입사일나이"
from employee
--55. employee 테이블에서 직원번호, 직원명, 주민번호, 다가올생일날(년-월-일), 생일까지 남은일수를 검색하면?
select emp_no, emp_name, jumin_num
        , -- 생일 - 오늘 > 0 이면 올해, 아니면 내년
        ...
        end||substr(jumin_num, 3, 4), 'yyyymmdd') - sysdate) "생일까지남은일"
from employee
--56. employee 테이블에서 직원번호, 직원명, 직급, 연봉(xxx,xxx,xxx만원)을 검색하면?
select emp_no, emp_name, jikup, to_char(salary, '999,999,999')||'만원' "연봉"
from employee
...
select emp_no, emp_name, round(sysdate-hire_date, 1) "근무일수"
        , round(months_between(sysdate, hire_date), 1) "근무개월수"
        , to_char(add_months(hire_date, 5), 'yyyy-mm-dd') "입사후5개월후날짜"
        , to_char(last_day(hire_date), 'yyyy-mm-dd') "입사달마지막날짜"
        , to_char(next_day(hire_date, 1), 'yyyy-mm-dd day', 'nls_date_language=korean') "일요일날짜"
from employee
--
...
select * from employee where hire_date between to_date('19900101', 'yyyy-mm-dd') and to_date('19991231','yyyy-mm-dd')
...
select cus_no, cus_name, emp_name
...
select emp_no, emp_name, dep_name from employee e inner join dept d on e.dep_no = d.dep_no
...
select c.cus_no, c.cus_name, c.jumin_num
from customer c inner join employee e on c.emp_no = e.emp_no
        and extract(year from sysdate) - to_number(decode(substr(e.jumin_num, 7, 1), '1', '19', '2', '19', '20')) + 1 >= 40
...
select emp_name, jikup, salary, floor(salary/(select sum(salary) from employee)*100)||'%' from employee
order by 4 desc
--
--112. 10번 부서 직원들이 관리하는 고객번호, 고객명, 직원번호를 검색하면?
select c.cus_no, c.cus_name, c.emp_no
from customer c inner join employee e on c.emp_no = e.emp_no and e.dep_no = 10
...
select emp_no, emp_name, (select dep_name from dept d where e.dep_no=d.dep_no), salary
from employee e
where salary = (select max(salary) from employee)
...
select dep_name
        ...
        , (select count(*) from employee e inner join customer c on e.emp_no = c.emp_no where e.dep_no = d.dep_no)
from dept d
--
...
select cus_no, cus_name, tel_num, (select emp_name from employee e where e.emp_no = c.emp_no and e.dep_no = 10)
...
select substr(extract(year from hire_date), 1, 3)||'0대'
        , decode(substr(jumin_num, 7, 1), '1', '남', '3', '남', '여')
        , count(*)
from employee
group by  substr(extract(year from hire_date), 1, 3)||'0대'
        , decode(substr(jumin_num, 7, 1), '1', '남', '3', '남', '여')
--
...
select e.emp_no, e.emp_name, ceil((sysdate-e.hire_date)/365)   "근무년차"
        , 60 - (extract(year from sysdate)
                 - to_number(decode(substr(e.jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(e.jumin_num, 1, 2))
                 + 1) "퇴직일까지남은년도"
...
select * from ( select rownum "RNUM", zxc.* from (
    select * from employee order by salary
) zxc where rownum <= 10 ) where rnum >= 5
--
...
select d.dep_no, d.dep_name, e.jikup, avg(salary) "AVG_SALARY"
from dept d left outer join employee e on d.dep_no = e.dep_no
...
update employee set salary = salary*1.05 where emp_no in(select emp_no from customer where emp_no is not null)
--
...
update employee set salary = salary*0.9 where emp_no in(
    select emp_no from (select rownum "RNUM", zxc.* from (
     select * from employee order by salary desc, decode(jikup, '사장', 1, '부장', 2, '과장', 3, '대리', 4, '주임', 5, 6)
                , hire_date, decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(jumin_num,1, 6)
     ) zxc where rownum <= 5  ) where rnum >= 2
)
--
