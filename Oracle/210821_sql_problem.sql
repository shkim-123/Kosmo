...
select emp_no "사원번호", emp_name "직원명", jikup "직급", salary||'만원' "연봉", hire_date "입사일"
from employee
...
select emp_no, emp_name, jikup, decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(jumin_num, 1, 1)||'0년대'
from employee
...
select emp_no, emp_name, to_char(hire_date, 'yyyy-mm-dd(day) q"분기" am hh:mi:ss', 'nls_date_language=korean')
from employee
--49-1. employee 테이블에서 직원번호, 직원명, 입사일(x년-x월-x일(요일) x분기 x시x분x초) 검색하면?
select emp_no, emp_name, to_char(hire_date, 'yyyy-mm-dd(dy) q"분기" hh"시"mi"분"ss"초"', 'nls_date_language=korean')
from employee
...
select emp_no, emp_name, ceil((sysdate-hire_date)/365) "근무년차" from employee
...
select emp_no, emp_name
    , to_char(to_date(decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')
        ||substr(jumin_num, 1, 6), 'yyyymmdd')+100, 'yyyy-mm-dd') "100일잔치"
from employee
...
select emp_no, emp_name
    , extract(year from sysdate)
     - to_number(decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(jumin_num, 1, 2)) + 1  "현재나이"
    , extract(year from hire_date)
     - to_number(decode(substr(jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(jumin_num, 1, 2)) + 1 "입사나이"
from employee
...
select emp_no, emp_name, jumin_num
    ...
    end||substr(jumin_num, 3, 4), 'yyyymmdd'), 'yyyy-mm-dd')   "다가올생일"
...
select emp_no, emp_name, jikup, to_char(salary, '999,999,999')||'만원' "연봉" from employee
...
select emp_no, emp_name
    ...
    , to_char(add_months(hire_date, 5), 'yyyy-mm-dd') "입사후5개월후"
    , to_char(last_day(hire_date), 'yyyy-mm-dd')   "입사한달의마지막날짜"
    , to_char(next_day(hire_date, 1), 'yyyy-mm-dd')  "입사기준일요일"
from employee
--
...
order by 4, decode(jikup, '사장', 1, '부장', 2, '과장', 3, '대리', 4, '주임', 5, 6)
...
select c.cus_no, c.cus_name, nvl(e.emp_no||'', '없음'), nvl(e.emp_name||'', '없음'), nvl(d.dep_name||'', '없음')
    , nvl(s.sal_grade_no||'', '없음'), nvl(e2.emp_name||'', '없음'), nvl(e2.jikup||'', '없음'), nvl(s2.sal_grade_no||'', '없음')
...
select emp_name||'직원' "이름", phone from employee
union
...
select emp_no, emp_name, (select dep_name from dept d where d.dep_no = e.dep_no) "부서명", salary
from employee e
...
select emp_no, emp_name, (select dep_name from dept d where e.dep_no = d.dep_no) "소속부서명"
from employee e
--117. 직원번호, 직원명, 연봉, 연봉 순위를 출력하면? 단, 연봉 순위를 오름차순 유지
select emp_no, emp_name, salary, (select count(*)+1 from employee e2 where e2.salary > e.salary)  "연봉순위"
from employee e
order by 4
--118. 직원번호, 직원명, 담당고객수를 출력하면?
select emp_no, emp_name, (select count(emp_no) from customer c where c.emp_no = e.emp_no) "담당고객수"
from employee e
--119. 부서명, 부서직원수, 부서담당고객수를 출력하면?
select d.dep_name
    , (select count(*) from employee e where e.dep_no = d.dep_no)  "부서직원수"
    , (select count(*) from employee e, customer c where e.dep_no = d.dep_no and c.emp_no = e.emp_no) "부서담당고객수"
from dept d
--
...
select emp_no, emp_name, jikup, jumin_num
    , (select count(*)+1 from employee e2
            where (e2.jikup <> e.jikup and
                ...
                < decode(substr(e.jumin_num, 7, 1), '1', '19', '2', '19', '20')||substr(e.jumin_num,1, 6)   )  "직급서열순위"
from employee e
order by 5
--
...
select cus_no, cus_name, tel_num
        , (select emp_name from employee e where e.emp_no=c.emp_no)  "담당직원명"
        , (select jikup from employee e where e.emp_no = c.emp_no)  "담당직원직급"
        , (select dep_no from employee e where e.emp_no = c.emp_no)  "부서번호"
from customer c
--
--122. 고객번호, 고객명, 고객전화번호, 담당직원명, 담당직원직급, 부서번호를 출력하면?
--<조건>고객정보는 모두 보이고 직원정보는 10번 부서만 보일 것. 단, 조인을 사용하지 말고 서브쿼리를 사용하십시오.
select cus_no, cus_name, tel_num
    , (select emp_name from employee e where e.emp_no=c.emp_no and e.dep_no=10) "담당직원명"
    , (select jikup from employee e where e.emp_no=c.emp_no and e.dep_no=10) "담당직원직급"
    , (select dep_no from employee e where e.emp_no=c.emp_no and e.dep_no=10) "부서번호"
from customer c
--
...
select dep_no, case when substr(jumin_num, 7, 1) in('1', '3') then '남' else '여' end "성별", sum(salary), avg(salary), count(*)
from employee
...
select dep_no, round(avg((sysdate-hire_date)/365), 1) "평균근무년수"
from employee
group by dep_no
--131. 입사분기별로 입사분기, 인원수를 출력하면?
select to_char(hire_date, 'q')||'분기', count(*)||'명' "인원수"
from employee
group by to_char(hire_date, 'q')||'분기'
--132. 입사연대별, 성별로 입사연대, 성별, 연대별입사자수 출력하면?
select substr(extract(year from hire_date), 1, 3)||'0년대' "년대"
...
select d.dep_no, d.dep_name, count(distinct e.emp_no) "직원수", count(c.emp_no) "고객수"
...
) where to_char(xday, 'day', 'nls_date_language=korean') not in('토요일', '일요일', '월요일')
--
...
) where to_char(xday, 'day', 'nls_date_language=korean') not in('토요일', '일요일')
--
...
insert into employee values((select nvl(max(emp_no), 0)+1 from employee), '장보고'
        , 40, '대리', 3500, to_date('2012-05-28', 'yyyy-mm-dd'), '8311091109310', '01092499215', 3)
--
...
update employee set salary = salary*0.9 where emp_no in(select emp_no from employee order by
    salary desc, decode(jikup, '사장', 1, '부장', 2, '과장', 3, '대리', 4, '주임', 5, 6), hire_date
...
insert into board(b_no, subject, writer, content, pwd, email, group_no, print_no, print_level)
    values((select nvl(max(b_no),0)+1 from board), '하이', '나나', 'asd', '123','add@navd.com'
...
-- b_no=3에 댓글 입력
insert into board(b_no, subject, writer, content, pwd, email, group_no, print_no, print_level)
    values((select nvl(max(b_no), 0)+1 from board), '하아이이', '히히', 'asss', '123', '222@naver.com'
...
-- 게시판의 모든 글 출력, 단순역순번호, 제목, 글쓴이, 작성일, 조회수,
-- 정렬 기준 그룹번호 내림차순, 출력순서번호 오름차순
-- 들여쓰기 표현
select x.cnt- rownum + 1 "NUM", zxc.* from (
    select lpad(' ', print_level*7, ' ' )||decode(print_level, 0, '', 'ㄴ')||subject "SUBJECT", writer
