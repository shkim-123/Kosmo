...
import com.naver.erp.BoardValidator;
...
//		}
		...
		//----------------------------------------------------------------
		Map<String, Integer> PagingMap = Util.getPagingNos(boardListAllCnt, boardSearchDTO.getSelectPageNo(), boardSearchDTO.getRowCntPerPage(), 10);
		//----------------------------------------------------------------
		// HashMap<String, Integer> 객체에 저장된 보정된 선택 페이지 번호를 
		// BoardSearchDTO 객체에 setSelectPageNo 메소드 호출로 덮어씌우기
		//----------------------------------------------------------------
		boardSearchDTO.setSelectPageNo(PagingMap.get("selectPageNo"));	
		...
		mav.addObject("pagingNos", PagingMap);
		//----------------------------------------------------------------
...
//		mav.addObject("pageNoCntPerPage", pageNoCntPerPage);
	...
	public ModelAndView goBoardRegForm(
			...
			//----------------------------------------------------------------
			@RequestParam(
					value="b_no"		// 파라미터명 설정, 이 파명에 해당하는 파값은 댓글 쓸 때 부모의 PK 번호이다.
